...
const getAIResponse = async (req, res) => {
  try {
    ...
    // If first message is a pitch deck request, inject formatting instruction
    const enhancedPrompt = isPitchDeckRequest(prompt || "")
...
const PITCH_DECK_INSTRUCTION = `When asked to create a pitch deck, respond with a complete, well-formatted pitch deck directly in your message using this exact structure:
...
---
*Generated by Perkart Intelligence*

Keep each section to 3-5 sentences. Be specific, data-driven, and compelling for investors. Use the context from the conversation to fill in details.`;
...
const chatAI = async (req, res) => {
  try {
    ...
    // If user is asking for a pitch deck, prepend formatting instruction
    const enhancedMessage = isPitchDeckRequest(message)
